...
/*
**
**@detail 首先创建一个list，取名为sections,记录一共有多少个section(一个section存储了CNN一层所需参数)
**        然后创建一个node，该node的指针指向一个新创建的section；char类型的指针指向.cfg文件中的某一行(line)
**        然后将该section的list指针执行一个新创建的node，该node的void指针指向一个kvp结构，kvp结构体中的key就是.
**        .cfg文件中的关键字(如：batch,subdivisions等)，val就是对应的值；如此循环
**
*/
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    /*fread是一个函数，它从文件流中读数据，最多读取count个项，每个项size个字节，如果调用成功返回实际取到的项个数(小于
    **或者等于count)，如果不成功或读到文件末尾返回0*/
