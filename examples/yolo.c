...
void train_yolo(char *cfgfile, char *weightfile)
{
    ...
    /*srand()和rand()配合使用产生伪随机数序列，rand()函数在产生随机数前，需要系统提供的生成伪随机数序列的种子
    **，rand根据这个种子的值产生一系列随机数。如果系统提供的种子没有变化，每次调用rand()函数生成的伪随机序列都
    **是一样的。
    */
    srand(time(0));

    /*第三个 参数是 cfg/yolo.train.cfg 
    **basecfg()这个函数把"cfg/yolo.train.cfg"变成了"yolo",然后用base指针指向"yolo"
    **basecfg()函数在utils.c中有定义：
    ...
    */
    ...
    /*jitter是什么意思呢？利用抖动(jittering)生成额外数据*/
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        /*pthread_join()函数，以阻塞的方式等待thread指定的线程结束。当函数返回时，被等待的线程的资源被收回。如果线程已经结束
        **那么该函数会立即返回
        */
        ...
        /*1000以内每100次保存一次权重，1000以外每1000次保存一次权重*/
